...
function __touchHandler() {
    ...
    this.__addEvent(container, 'touchmove', function (evt) {
        ...
        var movement = _this.movement,
            ...
            PULL_TO_REFRESH_MAX_OVERSCROLL = _this.PULL_TO_REFRESH_MAX_OVERSCROLL,
        ...
        if (movement.y && _this.__willOverscroll('y', y)) {
            var _factor = 2;
            var maxOverScrollSize = movement.y < 0 && options.pullToRefresh ? PULL_TO_REFRESH_MAX_OVERSCROLL : MAX_OVERSCROLL;
            if (movement.y < 0 && options.pullToRefresh || options.overscrollEffect === 'bounce') {
                _factor += Math.abs(10 * movement.y / maxOverScrollSize);
            }
